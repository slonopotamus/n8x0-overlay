...
src_compile() {
	...
	if use custom-gpio; then
		preloadsrc="${preloadsrc} ${FILESDIR}/preload_gpio.c"
	fi
	if ! use gconf; then
...
src_install() {
	...
	newinitd "${FILESDIR}/${iPN}.initd" "${PN}"
	newconfd "${FILESDIR}/${iPN}.confd" "${PN}"
}
